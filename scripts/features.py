...
class FeatureMapper(object):
    ...
    def vocab_init(fname, verbose=True):
        ...
        if verbose:
            ...
            print(
                "({} words, {} tags, {} nonterminal-chains)".format(
                    len(wdict),
                    len(tdict),
                    len(ldict),
                )
    ...
    def sentence_sequences(self, sentence):
        ...
        words = [
            self.wdict[w] if w in self.wdict else self.wdict[FeatureMapper.UNK]
            for (w, t) in sentence
        ]
        tags = [
            self.tdict[t] if t in self.tdict else self.tdict[FeatureMapper.UNK]
            for (w, t) in sentence
        ]
